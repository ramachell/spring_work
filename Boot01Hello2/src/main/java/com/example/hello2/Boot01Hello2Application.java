...
public class Boot01Hello2Application {
	...
	public static void main(String[] args) {
		...
		// 해당 객체로부터 Car type 객체 얻기
		Car car1 = ctx.getBean(Car.class);
		car1.drive();
		// getBean 또 호출해도 객체를 하나만 생성한다
		Car car2 = ctx.getBean(Car.class);
		car2.drive();

		if (car1 == car2) {
			System.out.println("car1 car2 같아요");
		}
		// 스프링이 관리하는 객체중 Remocon type 참조값
		// Remocon r1 = ctx.getBean(Remocon.class);
		// Remocon class 2개를 만들어놨으므로 (impl,tv) 이렇게 찾을수없음 이름으로 찾아야함
		Remocon r1 = (Remocon) ctx.getBean("myRemocon");

		r1.up();
		r1.down();
		// 스프링이 관리하는 객체중 myRemocon 이라는 이름의 객체 얻어와서 원래 type로 casting 해서 사용하기
